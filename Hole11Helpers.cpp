...
void h11FlatTurf(Figure& scene, float cx, float cy, float cz,
    ...
    Cube<4>* slab = new Cube<4>(h11pt(cx, cy, cz, o), 0.2f, width, depth);
...
void h11SlopeTurf(Figure& scene,
    ...
    Square<4> front(h11pt(fx, fy, fz, o), 0.2f, width);
    Square<4> back (h11pt(bx, by, bz, o), 0.2f, width);
...
void h11Wall(Figure& scene,
    ...
    Cube<4>* wall = new Cube<4>(h11pt(cx, cy, cz, o), h, w, d);
...
void h11Concrete(Figure& scene,
                 float cx, float cy, float cz,
                 float h, float w, float d, glm::vec3 o) {
    Cube<4>* block = new Cube<4>(h11pt(cx, cy, cz, o), h, w, d);
...
void h11VArm(Figure& scene,
    ...
    Square<4> tipFace (h11pt(tipX,  armCY, tipZ,  o), 0.45f, 0.15f);
    Square<4> baseFace(h11pt(baseX, armCY, baseZ, o), 0.45f, 0.15f);
