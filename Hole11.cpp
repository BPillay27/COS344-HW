...
void buildHole11(Figure& scene, glm::vec3 off) {
    ...
    const float Y_UPPER = 0.1f;   // surface at  0.2
    const float Y_MID   = -1.1f;  // surface at -1.0
    const float Y_LOWER = -2.1f;  // surface at -2.0
    ...
    h11FlatTurf(scene, 0.0f, Y_UPPER, -1.5f, HW, 3.0f, off);

    // Cliff retaining face at z = Z_CLIFF: fills the 1.2-unit drop visually
    {
    ...
    h11FlatTurf(scene, 0.0f, Y_MID, -6.0f, HW, 6.0f, off);
    ...
    h11FlatTurf(scene, 0.0f, Y_LOWER, -13.0f, HW, 4.0f, off);
    ...
    {
        float cy = Y_UPPER + WALL_H * 0.5f - 0.1f;
        h11Wall(scene, -LX, cy, -1.5f, WALL_H, WALL_T, 3.0f, off);
        h11Wall(scene,  LX, cy, -1.5f, WALL_H, WALL_T, 3.0f, off);
    }

    // Cliff-drop side patches: cover the 1.2-unit vertical gap in the wall
    {
    ...
    // Middle flat sides (z = Z_CLIFF to Z_MID_END)
    {
        ...
        h11Wall(scene, -LX, cy, -6.0f, WALL_H, WALL_T, 6.0f, off);
        h11Wall(scene,  LX, cy, -6.0f, WALL_H, WALL_T, 6.0f, off);
    }
    ...
    // Approximated at the average Y of the two ends so no gap opens.
    {
        ...
        h11Wall(scene, -LX, avgCY, -10.0f, WALL_H, WALL_T, 2.0f, off);
        h11Wall(scene,  LX, avgCY, -10.0f, WALL_H, WALL_T, 2.0f, off);
    }
    ...
    // Pitfall sides (z = Z_RAMP_END to Z_FAR)
    {
        ...
        h11Wall(scene, -LX, cy, -13.0f, WALL_H, WALL_T, 4.0f, off);
        h11Wall(scene,  LX, cy, -13.0f, WALL_H, WALL_T, 4.0f, off);
    }
    ...
    //           V arm                 V arm
    // -----------------------------------------------------------------------
    const float V_SURF = Y_MID + 0.1f;   // middle flat surface = -1.0

    // Left V — tip at outer wall, arms spread inward toward cup
    h11VArm(scene, -1.25f, Z_CUP - 0.3f, -0.55f, Z_CUP - 1.2f, V_SURF, off);
    h11VArm(scene, -1.25f, Z_CUP - 0.3f, -0.55f, Z_CUP + 1.2f, V_SURF, off);
    ...
    // -----------------------------------------------------------------------
    {
        ...
        glm::vec4 centre = h11pt(0.0f, cupCY, Z_CUP, off);
